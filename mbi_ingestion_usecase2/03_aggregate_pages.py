...
pages_table = config["ingestion"]["silver_layer"]["document_pages_table"]
...
formatted_elements_df = elements_df.withColumn(
    ...
    F.when(
        ...
        F.lower(F.col("element_type")) == "header",
        F.concat(F.lit("## "), F.coalesce(F.col("content"), F.lit("")))
    ).when(
...
# Join with silver pages table to bring in image_uri
pages_table_path = f"{catalog}.{schema}.{pages_table}"
pages_df = spark.table(pages_table_path).select("file_name", "page_id", "file_path", "image_uri")
aggregated_df = aggregated_df.join(pages_df, on=["file_name", "page_id"], how="left")
